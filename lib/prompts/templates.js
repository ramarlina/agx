...
const PROJECT_CONTEXT = [
  ...
  'Task state lives in the cloud API (goal, criteria, progress, learnings); `agx info`/`agx context` expose structured project metadata.',
...
agx retry <taskId>      # Retry failed task
agx reset <taskId>      # Reset task state
agx resume <task>       # Resume paused task
...
agx deps <task>                                    # Show depends_on + dependents
agx deps <task> --depends-on <a> --depends-on <b>  # Set dependencies
agx deps <task> --clear                            # Remove all dependencies
\`\`\`

Use task UUIDs, slugs, or \`agx task ls\` index references.

## Checking & Monitoring Tasks

\`\`\`bash
agx task ls             # List tasks
agx task logs <id> [-f] # View/tail task logs
agx task tail <id>      # Tail task logs
agx task stop <id>      # Stop a running task
agx task clear <id>     # Clear task history
agx task rm <id>        # Delete task
...
agx project get <id>                # Get project details
agx project create <name>           # Create project
agx project update <id> [fields]    # Update project
agx repo add . --project <proj-id>  # Analyze current repo and attach it to a project
agx project assign <task> <proj>    # Assign task to project
agx project unassign <task>         # Unassign task from project
agx new "goal" --project <proj-id>  # Create task in project
\`\`\`

## Gate Approvals

\`\`\`bash
agx approve <task>      # Approve a gated task/node
agx reject <task>       # Reject a gated task/node
\`\`\`

## Memory & Learnings

\`\`\`bash
agx memory list         # List agent memory
agx memory clear        # Clear agent memory
\`\`\`

Learnings (via API/MCP): task-level, project-level, or global scope.
...
agx daemon restart      # Restart daemon
agx daemon status       # Daemon status
agx daemon logs         # Daemon logs
agx daemon tail         # Tail daemon logs
agx daemon pick         # Pick next task from queue

agx board start         # Start dashboard
agx board stop          # Stop dashboard
agx board status        # Dashboard status
agx board open          # Open dashboard in browser
agx board logs          # Dashboard logs
\`\`\`

## Chat

\`\`\`bash
agx chat                # Interactive chat
agx chat start          # Start chat server
agx chat stop           # Stop chat server
agx chat send "msg"     # Send message
\`\`\`

## Mesh (Multi-Agent)

\`\`\`bash
agx mesh init           # Initialize mesh identity
agx mesh identity       # Show identity
agx mesh post "msg"     # Post to mesh feed
agx mesh react <id>     # React to a post
agx mesh comment <id>   # Comment on a post
agx mesh feed           # View mesh feed
agx mesh profile        # View profile
agx mesh self           # View own posts
agx mesh history        # Interaction history
agx mesh threads        # View threads
agx mesh activity       # Activity feed
agx mesh reflect        # Agent self-reflection
\`\`\`

## Containers

\`\`\`bash
agx container ls        # List containers
agx container logs <id> # Container logs
agx container stop <id> # Stop container
\`\`\`

## Workflows & Threads

\`\`\`bash
agx workflow list       # List workflows
agx workflow show <id>  # Show workflow details
agx thread status <id> <status>  # Update thread status
\`\`\`

## Admin & Utility

\`\`\`bash
agx audit               # Show audit logs
agx update status       # Check for updates
agx update --auto       # Enable auto-update
agx security status     # Security status
agx security rotate     # Rotate credentials
agx feedback            # Report bugs
agx telemetry on|off    # Toggle telemetry
\`\`\`

## Cloud API Endpoints

Tasks: CRUD, comments, logs, costs, heartbeat, history, dependencies, SSE streaming.
Execution Graphs: create/patch graph, start/pause/resume/restart/stop, replan, schedule, metrics, history, rollback, events.
Graph Nodes: start/stop/complete/fail/resume/verify per node, node comments.
Projects: CRUD, agents, variables, threads, skills, memory.
Participants: CRUD + reorder.
Chat: multiplexed agent responses with attachments.
Learnings: add/list/search at task/project/global scope.
Audit: log/list security events.
User Settings: provider/model preferences.
Providers: list available AI providers.
SSE: real-time task updates (/api/tasks/stream) and log streaming (/api/tasks/[id]/logs/stream).
...
-P, --provider <c|g|o|x>  Provider for new task
