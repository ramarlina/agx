...
function buildCloudTaskPromptFromContext(task) {
  ...
  // Keep most recent comments within budget
  let totalChars = commentLines.reduce((sum, l) => sum + l.length + 1, 0);
  while (totalChars > MAX_COMMENTS_CHARS && commentLines.length > 1) {
    totalChars -= commentLines[0].length + 1;
    commentLines.shift();
  }
  ...
  if (agentId && !task._noMemory) {
    try {
      ...
      // Fail-open: memory injection failure doesn't break prompt building
    }
  }
