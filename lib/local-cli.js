...
async function cmdRun({ taskSlug, projectSlug, stage = 'execute', engine = 'claude', json = false }) {
    ...
    let lockHandle;
    try {
        lockHandle = await storage.acquireTaskLock(taskRoot, { force: true });
    } catch (err) {
    ...
    try {
        ...
        // Update last_run immediately so `agx tail` can show the latest run's events
        // even before the run is finalized.
        await storage.updateLastRun(projectSlug, taskSlug, stage, run.run_id);
        ...
        // Build prompt
        const { promptText, totalBytes, event } = await storage.buildPrompt({
            projectSlug,
            taskSlug,
        });
