...
function decisionText(decision) {
  return [decision?.final_result, decision?.summary, decision?.explanation]
...
function enforceStageRequirement(decision, { stage, stagePrompt }) {
  ...
  const req = getStageRequirement(stage);
  // Only enforce explicit artifact requirements (ideation/planning today). For other stages, the
