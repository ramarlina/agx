...
// Flags that take no value (bare/boolean). The parser must not consume the
// next token as their value, otherwise positionals get swallowed.
const BARE_FLAGS = new Set(['--yes', '-y', '-h', '--help', '--json']);
...
function positionalArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    const tok = argv[i];
    if (tok.startsWith('-')) {
      // Only consume the next token as a value if this flag takes a value
      // (i.e. not in the bare-flag allowlist) and the next token is not itself a flag.
