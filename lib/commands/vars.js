function printVarsHelp(c) {
  console.log(`${c.bold}agx vars${c.reset} - Manage project environment variables`);
  console.log('');
  console.log('Usage:');
  console.log('  agx vars get <project> <KEY>          Print the value of a variable');
  console.log('  agx vars set <project> <KEY> <value>  Set a variable (creates or updates)');
  ...
  try {
    switch (subcmd) {
      ...
      case 'set': {
        ...
        if (!projectIdentifier || !key || subArgs.length < 3) {
          console.log(`${c.yellow}Usage:${c.reset} agx vars set <project> <KEY> <value>`);
          process.exit(1);
        }
        const KEY_PATTERN = /^[A-Z_][A-Z0-9_]*$/;
        if (!KEY_PATTERN.test(key)) {
          console.log(`${c.red}✗${c.reset} Invalid key "${key}" — must match [A-Z_][A-Z0-9_]*`);
          process.exit(1);
        }
        const proj = await resolveProjectByIdentifier(projectIdentifier);
        const result = await cloudRequest('POST', `/api/projects/${proj.id}/variables`, { key, value });
        if (result.error) {
          console.log(`${c.red}✗${c.reset} ${result.error}`);
          process.exit(1);
        }
        console.log(`${c.green}✓${c.reset} ${key} set`);
        process.exit(0);
        break;
      }
      case 'list':
