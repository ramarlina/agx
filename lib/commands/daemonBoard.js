...
async function maybeHandleDaemonBoardCommand({ cmd, args, ctx }) {
  ...
  if (cmd === 'daemon') {
    ...
    if (!subcmd || subcmd === 'run' || subcmd === '--run') {
      ...
      if (typeof ensureOrchestratorWorkerRunning === 'function') {
        await ensureOrchestratorWorkerRunning();
      }
    ...
    if (subcmd === 'start') {
      ...
      const workerPid = typeof isOrchestratorWorkerRunning === 'function' ? isOrchestratorWorkerRunning() : null;
      if (workerPid) {
        console.log(`${c.green}Orchestrator worker running${c.reset} (pid ${workerPid})`);
  ...
  if (cmd === 'board') {
    ...
    if (!subcmd || subcmd === 'start') {
      ...
      if (typeof stopOrchestratorWorker === 'function') {
