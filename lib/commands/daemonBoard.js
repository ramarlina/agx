...
async function maybeHandleDaemonBoardCommand({ cmd, args, ctx }) {
  ...
  // Board/UI commands
  if (cmd === 'board' || cmd === 'ui') {
    const commandName = cmd === 'ui' ? 'ui' : 'board';
    const subcmd = args[1];
    const wantsHelp = args.includes('--help') || args.includes('-h') || subcmd === 'help';
    ...
    if (!subcmd || subcmd === 'start') {
      ...
      console.log(`${c.bold}agx ${commandName}${c.reset} - Local UI runtime\n`);
      if (commandName === 'board') {
        console.log(`  ${c.dim}Alias:${c.reset} agx ui`);
      } else {
        console.log(`  ${c.dim}Canonical:${c.reset} agx board`);
      }
      console.log(`  agx ${commandName} start            Start local UI server`);
      console.log(`  agx ${commandName} stop             Stop local UI server`);
      console.log(`  agx ${commandName} status           Show status`);
      console.log(`  agx ${commandName} show             Print URL/log paths`);
      console.log(`  agx ${commandName} open             Open UI in browser`);
      console.log(`  agx ${commandName} logs             Show recent logs`);
      console.log(`  agx ${commandName} tail             Live tail logs`);
      console.log(`  agx ${commandName} share            Enable remote access via Tailscale`);
      console.log(`  agx ${commandName} share --off      Disable remote access`);
      console.log(`  agx ${commandName} share --status   Show remote access status`);
      console.log(`  agx ${commandName} start --no-open  Do not open browser`);
      ...
      if (pid) {
        ...
        console.log(`${c.dim}Logs: ${BOARD_LOG_FILE}${c.reset}`);
        // Show Tailscale remote access status
        try {
          const { isTailscaleServed } = require('./board');
          const isShared = await isTailscaleServed(port);
          if (isShared) {
            console.log(`${c.green}Remote access enabled${c.reset} via Tailscale`);
          }
        } catch { /* tailscale check is optional */ }
      } else {
        console.log(`${c.yellow}Board server not running${c.reset}`);
      }
      process.exit(0);
      ...
      const { handleBoardShare } = require('./board');
      await handleBoardShare({ args: args.slice(2), ctx });
    } else {
      console.log(`${c.red}Unknown ${commandName} command:${c.reset} ${subcmd}`);
      console.log(`${c.dim}Usage: agx ${commandName} [start|stop|status|show|open|logs|tail|share]${c.reset}`);
      process.exit(0);
    }
    return true;
  }
...
module.exports = { maybeHandleDaemonBoardCommand, openInBrowser };
