...
function printProjectHelp(c) {
  ...
  console.log('  agx project delete <id|slug>            Archive (soft delete) a project');
  ...
  try {
    switch (subcmd) {
      ...
      case 'archive': {
        const identifier = subArgs[0];
        if (!identifier) {
          console.log(`${c.yellow}Usage:${c.reset} agx project delete <id|slug>`);
          process.exit(1);
        }
        const targetProject = await resolveProjectByIdentifier(identifier);
        await cloudRequest('DELETE', `/api/projects/${targetProject.id}`);
        console.log(`${c.green}✓${c.reset} Archived project: ${targetProject.name} (${targetProject.slug || targetProject.id})`);
        process.exit(0);
        break;
      }
      default:
