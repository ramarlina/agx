...
function cmdInit(args, c, useJson) {
  ...
  logActivity(agentId, 'init', { meta: { voice, seed } });
...
function cmdPost(args, c, useJson) {
  ...
  logActivity(agentId, 'journal-post', { thread, meta: { entryId: entry.id } });
...
function cmdFeed(args, c, useJson) {
  ...
  // Journal posts from all agents (or just mine)
  ...
  if (!mine) {
    // ALL reactions across ALL agents (team-wide visibility)
    for (const a of listAllAgents()) {
      for (const r of readJsonl(path.join(AGENTS_DIR, a, 'reactions.jsonl'))) {
        if (new Date(r.t).getTime() >= sinceTs) {
          entries.push({ kind: 'reaction', ...r });
        }
      }
    }
    // ALL comments across ALL agents
    for (const a of listAllAgents()) {
      for (const cm of readJsonl(path.join(AGENTS_DIR, a, 'comments.jsonl'))) {
        if (new Date(cm.t).getTime() >= sinceTs) {
          entries.push({ kind: 'comment', ...cm });
        }
      }
    }
  }
...
function cmdActivity(args, c, useJson) {
  const agentId = getFlag(args, '--agent');
  const all = hasFlag(args, '--all');

  if (!agentId && !all) {
    errorOut('VALIDATION', 'Usage: agx mesh activity --agent <id> | --all [--limit N] [--since ISO] [--action <type>]', useJson);
    process.exit(2);
  }

  const limit = parseInt(getFlag(args, '--limit') || '50', 10);
  const since = getFlag(args, '--since');
  const actionFilter = getFlag(args, '--action');
  const sinceTs = since ? new Date(since).getTime() : 0;
  ...
  for (const id of agents) {
    if (!all) resolveAgent(id);
    for (const e of readJsonl(path.join(AGENTS_DIR, id, 'activity.jsonl'))) {
  ...
  output(events.slice(0, limit), useJson);
}
...
async function maybeHandleMeshCommand({ cmd, args, ctx }) {
  ...
  try {
    switch (subcommand) {
      ...
      case 'activity': cmdActivity(args, c, useJson); break;
      case 'reflect': cmdReflect(args, c, useJson); break;
      default:
        console.log(`Usage: agx mesh <command> [options]`);
        console.log('');
        ...
        console.log('  feed      Show team activity feed (all agents\' posts, reactions, comments)');
        console.log('  activity  Show raw activity log (every action)');
