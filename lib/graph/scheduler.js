...
function normalizeAllowedNodeIds(allowedNodeIds) {
  if (!allowedNodeIds) return null;
  const values = Array.isArray(allowedNodeIds) ? allowedNodeIds : [allowedNodeIds];
...
function tick(graph, options = {}) {
  ...
  const events = [];
  const allowedNodeIds = normalizeAllowedNodeIds(options.allowedNodeIds);
  ...
  for (const nodeId of Object.keys(nextGraph.nodes)) {
    ...
    // Root/fork/join nodes auto-complete when deps are satisfied
    if (node.type !== "work" && node.type !== "gate") {
      if (canRunNode(nextGraph, nodeId)) {
        const previous = node.status;
        node.status = "done";
        node.completedAt = node.completedAt || now;
        events.push({
