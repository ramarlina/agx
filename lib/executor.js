...
const { createCheckpoint } = require('./storage/checkpoints');
...
async function executeTask(options) {
  ...
  const persistCheckpoint = async (label) => {
    ...
    try {
      ...
      try {
        const { syncCheckpointToCloud } = require('./cloud-sync');
        // Get cloud task ID from options if available
        const cloudTaskId = options.cloudTaskId;
        if (cloudTaskId && checkpoint) {
          syncCheckpointToCloud({ taskId: cloudTaskId, checkpoint, onLog }).catch(() => {});
        }
      } catch {
        // Cloud sync not available or failed - continue
      }
    } catch (err) {
  ...
  const handleBlocked = async (blockedInfo) => {
    ...
    try {
      await createCheckpoint(projectSlug, taskSlug, {
        ...richBlockedState,
