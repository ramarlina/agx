...
/**
 * Resolve the absolute path for an engine command.
 * 1. Check stored providerPaths from config
 * 2. Try resolveCommand (current PATH)
 * 3. Try common fallback directories
 * 4. Fall back to bare command name
 */
function resolveEngineCmd(cmd) {

  // 1. Stored path from onboarding
  try {
    const { loadConfig } = require('./cli/configStore');
    const config = loadConfig();
    const stored = config?.providerPaths?.[cmd];
  ...
  // 3. Common fallback dirs (platform-aware)
  const home = os.homedir();
  const isWin = process.platform === 'win32';
  const exe = isWin ? `${cmd}.cmd` : cmd;
  ...
  for (const dir of fallbackDirs) {
    ...
    // On Windows, also try without .cmd extension
    if (isWin) {
      const plain = path.join(dir, cmd);
      if (fs.existsSync(plain)) return plain;
    }
  }

  // 4. Bare command (last resort)
  return cmd;
}
...
async function executeTask(options) {
  ...
  const workDir = path.join(CONFIG_DIR, 'workdir', `agx-${taskId}-${stage}`);
  ...
  return new Promise((resolve, reject) => {
    ...
    const resolvedCmd = resolveEngineCmd(engineConfig.cmd);
    const proc = execa(resolvedCmd, args, {
      cwd: workDir,
