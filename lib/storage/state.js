...
async function appendEventToLastRun(projectSlug, taskSlug, event) {
    if (!event) return false;
    try {
        const { runPaths } = require('./paths');
        const lastRun = await readLastRun(projectSlug, taskSlug);
        if (!lastRun?.overall?.stage || !lastRun?.overall?.run_id) return false;
        const paths = runPaths(projectSlug, taskSlug, lastRun.overall.stage, lastRun.overall.run_id);
...
async function updateTaskState(projectSlug, taskSlug, updates) {
    ...
    // Emit per-run STATE_UPDATED events for fields that changed (best-effort).
    for (const field of Object.keys(allowedUpdates)) {
        ...
        await appendEventToLastRun(projectSlug, taskSlug, stateUpdatedEvent({
            field,
            old_value: oldVal,
            new_value: newVal,
        }));
...
async function writeWorkingSet(projectSlug, taskSlug, content, options = {}) {
    ...
    // Create event if content was rewritten
    const event = rewritten
        ? workingSetRewrittenEvent({
    ...
    if (event) {
        await appendEventToLastRun(projectSlug, taskSlug, event);
    }
...
async function addPendingApproval(projectSlug, taskSlug, { action, reason }) {
    ...
    await appendEventToLastRun(projectSlug, taskSlug, approvalRequestedEvent({
        id: request.id,
...
async function approveRequest(projectSlug, taskSlug, approvalId) {
    ...
    await writeApprovals(projectSlug, taskSlug, approvals);
    // Local task lifecycle uses "pending" as the queued/runnable state.
    await updateTaskState(projectSlug, taskSlug, { status: 'pending' });

    await appendEventToLastRun(projectSlug, taskSlug, approvalGrantedEvent({ id: request.id }));
...
async function rejectRequest(projectSlug, taskSlug, approvalId) {
    ...
    await appendEventToLastRun(projectSlug, taskSlug, approvalRejectedEvent({
        id: request.id,
        reason: request.reason,
    }));
