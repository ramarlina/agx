...
const RUN_ID_RAND_BYTES = 2; // 4 hex chars
...
function validateRunId(runId) {
    ...
    // Match YYYYMMDD-HHMMSS-xxxx format
    const pattern = /^\d{8}-\d{6}-[a-f0-9]{4}$/;
    if (!pattern.test(runId)) {
        return { valid: false, error: 'Run ID must be in format YYYYMMDD-HHMMSS-xxxx' };
    }
    return { valid: true };
}
