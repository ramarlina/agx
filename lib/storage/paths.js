...
function projectRoot(projectSlug) {
    ...
    if (!validation.valid) {
        throw new Error(`Invalid project slug: ${validation.error} (got ${typeof projectSlug}: ${JSON.stringify(projectSlug)})`);
    }
...
/**
 * Get the graph.json path for a task.
 * @param {string} projectSlug
 * @param {string} taskSlug
 * @returns {string}
 */
function graphJsonPath(projectSlug, taskSlug) {
    return path.join(taskRoot(projectSlug, taskSlug), 'graph.json');
}
