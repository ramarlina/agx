 ...
 * @param {string} [params.runId] - Optional existing run id to reuse (creates another stage folder under the same run container)
 * @param {string} params.engine
 * @param {string} [params.model]
 * @param {GitMeta} [params.git]
 * @returns {Promise<RunHandle>}
 */
async function createRun({ projectSlug, taskSlug, stage, runId: explicitRunId, engine, model, git }) {
    // Validate stage
    ...
    // Prevent accidental clobbering if this stage/run already exists.
    if (await fileExists(paths.meta)) {
        throw new Error(`Run already exists: stage=${stage} run_id=${runId}`);
    }
...
async function listRuns(projectSlug, taskSlug, options = {}) {
    ...
    // New layout: <task_slug>/<run_id>/<stage>/
    try {
        ...
        for (const entry of entries) {
            ...
            for (const stage of stages) {
                const stageRoot = runRoot(projectSlug, taskSlug, stage, runId);
                try {
                    const st = await fs.promises.stat(stageRoot);
                ...
                if (!meta && !hasDecision) continue;

                runs.push({ stage, run_id: runId, meta, hasDecision });
            }
        }
    } catch (err) {
    ...
    // Legacy layout: <task_slug>/<stage>/<run_id>/
    for (const stage of stages) {
        ...
        try {
            ...
            for (const entry of entries) {
                ...
                const paths = runPathsLegacy(projectSlug, taskSlug, stage, runId);
                const meta = await readJsonSafe(paths.meta);
                const hasDecision = await fileExists(paths.decision);
                if (!meta && !hasDecision) continue;
...
async function closeIncompleteRun(projectSlug, taskSlug, stage, runId) {
    const newPaths = runPaths(projectSlug, taskSlug, stage, runId);
    const legacyPaths = runPathsLegacy(projectSlug, taskSlug, stage, runId);
    const paths = (await fileExists(newPaths.meta)) || (await fileExists(newPaths.events)) ? newPaths : legacyPaths;
...
async function readDecision(projectSlug, taskSlug, stage, runId) {
    const newPaths = runPaths(projectSlug, taskSlug, stage, runId);
    const legacyPaths = runPathsLegacy(projectSlug, taskSlug, stage, runId);
    const paths = (await fileExists(newPaths.decision)) || (await fileExists(newPaths.meta)) ? newPaths : legacyPaths;
...
async function readRunMeta(projectSlug, taskSlug, stage, runId) {
    const newPaths = runPaths(projectSlug, taskSlug, stage, runId);
    const legacyPaths = runPathsLegacy(projectSlug, taskSlug, stage, runId);
    const paths = (await fileExists(newPaths.meta)) ? newPaths : legacyPaths;
...
async function gcRuns(projectSlug, taskSlug, options = {}) {
    ...
    for (const stage of VALID_STAGES) {
        ...
        for (const run of toDelete) {
            const newPath = runRoot(projectSlug, taskSlug, stage, run.run_id);
            const legacyPath = runRootLegacy(projectSlug, taskSlug, stage, run.run_id);
            const runPath = await fileExists(newPath) ? newPath : legacyPath;
