...
async function createCheckpoint(projectSlug, taskSlug, data = {}) {
    ...
    // Apply limits to prevent unbounded growth
    const limitArray = (arr, max) => Array.isArray(arr) ? arr.slice(0, max) : arr;
    
    const limitedData = { ...data };
    if (limitedData.plan && Array.isArray(limitedData.plan.steps)) {
        limitedData.plan = { ...limitedData.plan, steps: limitArray(limitedData.plan.steps, MAX_PLAN_STEPS) };
    }
    if (Array.isArray(limitedData.criteria)) {
        limitedData.criteria = limitArray(limitedData.criteria, MAX_CRITERIA);
    }
    if (Array.isArray(limitedData.constraints)) {
        limitedData.constraints = limitArray(limitedData.constraints, MAX_CONSTRAINTS);
    }
    if (Array.isArray(limitedData.dontRepeat)) {
        limitedData.dontRepeat = limitArray(limitedData.dontRepeat, MAX_DONT_REPEAT);
    }
