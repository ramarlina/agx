...
async function createProject(flags, cloudRequestFn) {
  ...
  const createPayload = {};
  if (body.name) createPayload.name = body.name;
  if (body.description) createPayload.description = body.description;
  if (body.repos) createPayload.repos = body.repos;

  const createResult = await cloudRequestFn('POST', '/api/projects', createPayload);
  const createdProject = createResult?.project;

  const patchPayload = {};
  if (body.slug) patchPayload.slug = body.slug;
  if (body.metadata) patchPayload.metadata = body.metadata;
  if (body.ci_cd_info) patchPayload.ci_cd_info = body.ci_cd_info;
  if (body.workflow_id) patchPayload.workflow_id = body.workflow_id;

  if (createdProject?.id && Object.keys(patchPayload).length) {
    const patchResult = await cloudRequestFn('PATCH', `/api/projects/${createdProject.id}`, patchPayload);
    return patchResult;
  }
