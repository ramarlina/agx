...
function buildTaskStatusLines(task, logs, options = {}) {
  ...
  // Graph / execution nodes (injected via options.graph)
  const graphRaw = options.graph;
  const graph = graphRaw?.graph || graphRaw; // unwrap { graph: {...} } envelope
  const graphNodes = graph?.nodes;
  if (graph && graphNodes && typeof graphNodes === 'object') {
    const entries = Array.isArray(graphNodes)
      ? graphNodes.map(n => [n.id || '?', n])
      : Object.entries(graphNodes);
    if (entries.length > 0) {
      lines.push(`${colors.cyan}Execution Graph:${colors.reset}  ${colors.dim}v${graph.version || graph.graphVersion || '?'}${colors.reset}`);
      const statusIcon = (s) => {
        if (s === 'done' || s === 'completed') return `${colors.green}✓${colors.reset}`;
        if (s === 'passed') return `${colors.green}✓${colors.reset}`;
        if (s === 'running' || s === 'in_progress') return `${colors.yellow}▶${colors.reset}`;
        if (s === 'awaiting_human') return `${colors.magenta}⏸${colors.reset}`;
        if (s === 'failed') return `${colors.red}✗${colors.reset}`;
        return `${colors.dim}○${colors.reset}`;
      };
      for (const [nodeId, node] of entries) {
        const label = node.title || node.label || node.type || nodeId;
        let detail = '';
        if (node.actualMinutes) detail += ` ${colors.dim}${node.actualMinutes}m${colors.reset}`;
        if (node.attempts) detail += ` ${colors.dim}(attempt ${node.attempts}/${node.maxAttempts || '?'})${colors.reset}`;
        lines.push(`  ${statusIcon(node.status)} ${label}  ${colors.dim}${nodeId}${colors.reset}${detail}`);
        if (node.error) {
          const errMsg = typeof node.error === 'string' ? node.error : node.error.message || JSON.stringify(node.error);
          lines.push(`    ${colors.red}${errMsg.slice(0, 200)}${colors.reset}`);
        }
        if (node.output?.summary && (node.status === 'done' || node.status === 'completed')) {
          const summary = String(node.output.summary).replace(/\x1b\[[0-9;]*m/g, '').trim();
          if (summary && !summary.startsWith('[cloud]')) {
            lines.push(`    ${colors.dim}${summary.slice(0, 200)}${colors.reset}`);
          }
        }
      }
      lines.push('');
...
async function showCloudTaskStatus({
  ...
  const [logs, graph] = await Promise.all([
    fetchLogs({ cloudRequest, taskId: resolvedTaskId, tail: 10, logger, colors }),
    cloudRequest('GET', `/api/tasks/${encodeURIComponent(resolvedTaskId)}/graph`).catch(() => null),
  ]);

  const lines = buildTaskStatusLines(task, logs, { colors, formatTimestamp: formatFn, graph });
