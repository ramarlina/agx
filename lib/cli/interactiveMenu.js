  ...
  const handleSelect = async () => {
      ...
      if (item.id === 'chat') {
        ...
        try {
          await startChat({
            ...
            model: selectedProvider === 'ollama' ? config?.ollama?.model || 'llama3.2:3b' : null,
          });
        } catch (err) {
          console.log(`${c.red}Chat error:${c.reset} ${err.message}`);
        }
        process.exit(0);
      }
      return;
    ...
    if (menuState === 'daemon') {
      ...
      if (item.id === 'status') {
        ...
        const workerPid = isOrchestratorWorkerRunning();
        if (workerPid) {
