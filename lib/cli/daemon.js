...
const PACKAGED_BOARD_ROOT = path.join(AGX_PKG_ROOT, 'cloud-runtime', 'standalone');
const LOCAL_BOARD_APP_DIR = path.join(AGX_PKG_ROOT, 'apps', 'local');
const CUSTOM_BUNDLED_BOARD_ENTRY = 'agx-server.js';

let cachedPackagedBoardDir;
function resolvePackagedBoardDir() {
  if (cachedPackagedBoardDir !== undefined) return cachedPackagedBoardDir;
  ...
  const maxDepth = 8;
  const stack = [{ dir: PACKAGED_BOARD_ROOT, depth: 0 }];
  while (stack.length) {
    ...
    if (isStandaloneAppDir(dir)) {
      cachedPackagedBoardDir = dir;
      return cachedPackagedBoardDir;
    }
...
function resolveBoardDir() {
  ...
  const override = process.env.AGX_BOARD_WORKER_DIR || process.env.AGX_CLOUD_WORKER_DIR;
  ...
  for (const dir of [LOCAL_BOARD_APP_DIR]) {
  ...
  const packaged = resolvePackagedBoardDir();
...
function getBundledBoardServerEntry(dir) {
  const customEntry = path.join(dir, CUSTOM_BUNDLED_BOARD_ENTRY);
  if (fs.existsSync(customEntry)) {
    return CUSTOM_BUNDLED_BOARD_ENTRY;
  }
  return 'server.js';
}

function getBoardPort() {
  const apiUrl = process.env.AGX_BOARD_URL || process.env.AGX_CLOUD_URL || 'http://localhost:41741';
  try {
...
async function ensureBoardRunning() {
  ...
  if (!boardInfo) {
    console.error(`${c.red}Board runtime not found.${c.reset} Ensure apps/local exists in the agx repo or build the bundled runtime.`);
    console.log(`${c.dim}Tip: set AGX_BOARD_WORKER_DIR=/path/to/agx/apps/local (legacy alias: AGX_CLOUD_WORKER_DIR)${c.reset}`);
    _boardEnsured = false;
    return;
  }
  ...
  // Board runs on SQLite — pass db paths for shared state.
  // In bundled mode, process.cwd() is the standalone app dir (e.g. /usr/lib/...)
  // which may not be writable. Point all db paths to ~/.agx/ instead.
  const dbPath = process.env.AGX_DB_PATH || path.join(CONFIG_DIR, 'agx.db');
  const queueDbPath = process.env.SQLITE_QUEUE_PATH || path.join(CONFIG_DIR, 'agx-queue.db');
  const boardDbPath = process.env.SQLITE_DB_PATH || path.join(CONFIG_DIR, 'agx-board.db');
  const boardEnv = {
    ...process.env,
    AGX_DB_PATH: dbPath,
    SQLITE_QUEUE_PATH: queueDbPath,
    SQLITE_DB_PATH: boardDbPath,
    PORT: String(port),
  ...
  try {
    if (boardInfo.mode === 'bundled') {
      const bundledEntry = getBundledBoardServerEntry(boardInfo.dir);
      proc = execa('node', [bundledEntry], {
        cwd: boardInfo.dir,
        detached: true,
...
module.exports = {
  ...
  stopDaemon,
  resolveBoardDir,
  resolvePackagedBoardDir,
  resolvePackagedAgxCloudDir: resolvePackagedBoardDir,
  getBundledBoardServerEntry,
