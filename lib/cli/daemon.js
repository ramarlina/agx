...
function isOrchestratorWorkerRunning() {
...
function ensureSchemaInitialized(dbUrl) {
  ...
  let psqlResult;
  try {
    psqlResult = execa.sync('psql', [dbUrl], { input: initSql, timeout: 60000, encoding: 'utf8', reject: false });
  } catch (err) {
    console.log(`${c.yellow}Schema init skipped${c.reset}: ${err?.message || 'psql is not available'}`);
    return;
  }
  if (psqlResult.exitCode !== 0) {
    const stderr = String(psqlResult.stderr || '').trim();
...
function failPostgresSetup(message) {
  const detail = String(message || '').trim();
  if (detail) {
    console.error(`${c.red}${detail}${c.reset}`);
  }
  console.error(`${c.yellow}Unable to bootstrap local Postgres for agx board.${c.reset}`);
  console.error(`${c.dim}Set DATABASE_URL to an existing Postgres instance, or set AGX_CLOUD_URL to a running agx-cloud backend.${c.reset}`);
  process.exit(1);
}

function resolveConfiguredDatabaseUrl() {
  const boardEnv = loadBoardEnv();
  const boardDbUrl = String(boardEnv.DATABASE_URL || '').trim();
  const envDbUrl = String(process.env.DATABASE_URL || '').trim();
  const selected = boardDbUrl || envDbUrl;
  if (!selected) return null;
  if (!boardDbUrl && envDbUrl) {
    saveBoardEnvValue('DATABASE_URL', envDbUrl);
  }
  return selected;
}

async function ensurePostgresReady() {
  const configuredDbUrl = resolveConfiguredDatabaseUrl();
  if (configuredDbUrl) {
    try {
      const dbUrl = new URL(configuredDbUrl);
      const host = dbUrl.hostname;
      const port = dbUrl.port || '5432';
      const result = execa.sync('pg_isready', ['-h', host, '-p', port], { timeout: 3000, reject: false });
      if (result.exitCode !== 0) {
        const stderr = String(result.stderr || '').trim();
        console.log(`${c.yellow}DATABASE_URL check failed${c.reset}${stderr ? `: ${stderr}` : ''}`);
      }
    } catch (err) {
      if (err?.code === 'ENOENT') {
        console.log(`${c.dim}pg_isready not found; continuing with configured DATABASE_URL.${c.reset}`);
      } else if (err instanceof TypeError) {
        failPostgresSetup(`Invalid DATABASE_URL: ${configuredDbUrl}`);
      } else {
        console.log(`${c.yellow}DATABASE_URL probe failed${c.reset}: ${err?.message || err}`);
      }
    }
    ensureSchemaInitialized(configuredDbUrl);
  ...
  if (isInteractivePromptAvailable()) {
    ...
    choice = answer.trim() || '2';
  } else {
    console.log(`${c.dim}Non-interactive session detected; defaulting to Docker auto-start.${c.reset}`);
  }

  if (choice === '1') {
    if (!isInteractivePromptAvailable()) {
      failPostgresSetup('Cannot prompt for DATABASE_URL in non-interactive mode.');
    }
  ...
  let dockerResult;
  try {
    dockerResult = execa.sync('docker', [
      'run', '-d',
      '--name', 'agx-postgres',
      '-e', 'POSTGRES_DB=agx',
      '-e', 'POSTGRES_USER=agx',
    ...
    failPostgresSetup(`Failed to start Docker postgres: ${err?.message || err}`);
  }
  ...
  if (dockerResult.exitCode !== 0) {
    ...
    if (stderr.includes('already in use')) {
      try {
        const startResult = execa.sync('docker', ['start', 'agx-postgres'], { timeout: 10000, reject: false, encoding: 'utf8' });
        if (startResult.exitCode !== 0) {
          failPostgresSetup(`Failed to start existing agx-postgres container: ${String(startResult.stderr || '').trim() || 'unknown error'}`);
        }
      } catch (err) {
        failPostgresSetup(`Failed to start existing agx-postgres container: ${err?.message || err}`);
      }
    } else {
      failPostgresSetup(`Failed to start postgres: ${stderr || 'unknown docker error'}`);
    }
  }
  ...
  while (Date.now() < deadline) {
    let check;
    try {
      check = execa.sync('docker', ['exec', 'agx-postgres', 'pg_isready', '-U', 'agx'], { timeout: 3000, reject: false, encoding: 'utf8' });
    } catch (err) {
      failPostgresSetup(`Failed while checking postgres readiness: ${err?.message || err}`);
    }
    if (check.exitCode === 0) {
      ready = true;
      break;
  ...
  if (!ready) {
    failPostgresSetup('Timed out waiting for Docker postgres to become ready.');
  }
...
function startOrchestratorWorker() {
  const existingPid = isOrchestratorWorkerRunning();
  if (existingPid) {
...
async function ensureOrchestratorWorkerRunning() {
  try {
    const boardEnv = loadBoardEnv();
    if (!boardEnv.DATABASE_URL) {
  ...
  return startOrchestratorWorker();
}

async function stopOrchestratorWorker() {
  const pid = isOrchestratorWorkerRunning();
  if (!pid) {
...
function startDaemon(options = {}) {
  ...
  void ensureOrchestratorWorkerRunning();
...
async function stopDaemon() {
  ...
  const workerStopped = await stopOrchestratorWorker();
  const boardStopped = await stopBoard();
  return daemonStopped || workerStopped || boardStopped;
}
...
module.exports = {
  ...
  isDaemonRunning,
  isBoardRunning,
  isOrchestratorWorkerRunning,
  stopDaemonProcessTree,
  ensureBoardRunning,
  stopBoard,
  ensureOrchestratorWorkerRunning,
