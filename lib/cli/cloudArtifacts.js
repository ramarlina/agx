...
const { buildLegacyInstructionFooter } = require('../prompts/cloudTask');
...
function mapCloudStageToLocalStage(stage) {
  ...
  if (raw === 'intake' || raw === 'planning') return 'plan';
  ...
  return {
    ...
    async flush() {
      ...
      try {
        ...
        if (run?.finalized || msg.includes('finalized run')) {
          return;
        }
        throw err;
      }
...
function buildFullDaemonPromptContext(task, options = {}) {
  ...
  const extract = (typeof extractSection === 'function')
    ? extractSection
    : (markdown, heading) => {
      if (!markdown) return '';
      const h = String(heading || '').trim();
  ...
  const plan = extract(task?.content, 'Plan');
  const todo = extract(task?.content, 'Todo') || extract(task?.content, 'TODO');
  const checkpoints = extract(task?.content, 'Checkpoints');
  const learnings = extract(task?.content, 'Learnings');
  ...
  // Build project repos section so the agent knows where to cd
  const projectObj = task?.project && typeof task.project === 'object' ? task.project : null;
  ...
  const reposSection = Array.isArray(repos) && repos.length
    ? repos.map((r) => {
      const parts = [r.name || 'unnamed'];
      if (r.path) parts.push(`path: ${r.path}`);
      if (r.git_url) parts.push(`git: ${r.git_url}`);
  ...
  let prompt = hasTaskPrompt ? task.prompt.trim() : `## Cloud Task Context
...
${reposSection ? `\n## Project Repos\n\nYou are starting in the home directory. cd into the appropriate repo before working.\n${reposSection}\n` : ''}

User Request: 
"""
${task?.title || ''}
${resolveTaskPromptBody(task)}
---
Task Thread:
${commentsSection}
"""
...
`;
  ...
  // Inject agent memory from local SQLite (per-project past learnings)
  const agentId = task?.project_slug || task?.project?.slug || task?.project_name || task?.project?.name || null;
  if (agentId && !task?._noMemory) {
    try {
      const { buildMemoryBlock } = require('../memory/inject');
  ...
  prompt += `\n\n${buildLegacyInstructionFooter({ finalPrompt: iterationPrompt })}`;
