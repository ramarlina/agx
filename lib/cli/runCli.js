  ...
  agx container logs         # See what's happening`);
    ...
    if (options.cloudTaskId) {
      try {
        ...
        // For codex: use --cd and --add-dir to point at project repos.
        if (command === 'codex') {
          const repos = (task?.project_context?.repos || []).filter((r) => r?.path);
          if (repos.length > 0) {
            translatedArgs.push('--cd', repos[0].path);
            for (let ri = 1; ri < repos.length; ri++) {
              translatedArgs.push('--add-dir', repos[ri].path);
            }
          }
        }
