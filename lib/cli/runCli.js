...
const SWARM_PROVIDERS = ['claude', 'gemini', 'ollama', 'codex', 'kimi'];
...
// Placeholder for optional orchestrator signaling hook (legacy code paths call it).
const signalOrchestratorTask = undefined;
...
async function checkOnboarding() {
  ...
  async function cloudRequest(method, endpoint, body = null) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error((data?.error || `HTTP ${response.status}`) + issues);
      }
  ...
  async function runTaskInline(rawTaskId, options = {}) {
    const { resetFirst = false, forceSwarm = false, fromStage = null, resumePrompt = '', noMemory = false } = options;
    ...
    if (noMemory) effectiveTask._noMemory = true;
  ...
  async function runCloudDaemonTask(task) {
    ...
    // --- Agent memory extraction (fail-open) ---
    if (task?._noMemory) {
      logger.log('system', '[memory] skipped (--no-memory)\n');
    } else try {
      const { extractAndStoreMemories } = require('../memory/extract');
      const { callClaude } = require('../memory/llm');
      const memoryAgentId = projectSlug || 'default';
      const memoryOutcome = decisionPayload.decision === 'done' ? 'success' : 'failed';
      const memorySummary = [
        decisionPayload.summary,
        decisionPayload.explanation,
        decisionPayload.final_result,
      ].filter(Boolean).join('\n');
      ...
      if (memorySummary.trim()) {
        await extractAndStoreMemories({
          ...
          llmCall: (prompt) => callClaude(prompt),
          onLog: (msg) => logger.log('system', `${msg}\n`),
        });
      }
    } catch (err) {
      // Fail-open: never block task completion
      logger.log('error', `[memory] extraction failed (non-fatal): ${err?.message || err}\n`);
    }
  ...
  async function runCloudDaemonLoop(options = {}) {
    ...
    // Enforce operational limits (see agx-cloud/docs/LIMITS.md)
    const MAX_WORKERS_LIMIT = Number(process.env.AGX_MAX_WORKERS) || 10;
    if (maxWorkers > MAX_WORKERS_LIMIT) {
      console.error(
        `${c.red}✗${c.reset} Requested ${maxWorkers} workers exceeds MAX_WORKERS limit (${MAX_WORKERS_LIMIT}).`
        + `\n  See docs/LIMITS.md — exceeding this risks SQLITE_BUSY errors.`
  ...
  // agx schedule <subcommand>
  if (cmd === 'schedule') {
    const { handleSchedule } = require('../commands/schedule');
    await handleSchedule({
      args: args.slice(1),
      ctx: { c, cloudRequest, loadCloudConfig },
    });
    return true;
  }

  if (cmd === 'narrate-prs') {
    const { handleNarratePrs } = require('../commands/narratePrs');
    await handleNarratePrs({ args: args.slice(1), ctx: { c } });
    return true;
  }
  ...
  {
    const { maybeHandleTeamsCommand } = require('../commands/teams');
    const handledTeams = await maybeHandleTeamsCommand({
      cmd,
      args,
      ctx: {
        c,
    ...
    if (handledTeams) return true;
  }

  {
    const { maybeHandleRepoCommand } = require('../commands/repo');
    const handledRepo = await maybeHandleRepoCommand({
      cmd,
      args,
      ctx: {
        c,
    ...
    if (handledRepo) return true;
  }

  {
    const { maybeHandleWorkspaceCommand } = require('../commands/workspace');
    const handledWorkspace = await maybeHandleWorkspaceCommand({
      cmd,
      args,
      ctx: {
        c,
    ...
    if (handledWorkspace) return true;
  }

  {
    const { maybeHandleVarsCommand } = require('../commands/vars');
    const handledVars = await maybeHandleVarsCommand({
      cmd,
      args,
      ctx: {
        c,
  ...
  {
    const { maybeHandleGateCommand } = require('../commands/gate');
    const handledGate = await maybeHandleGateCommand({ cmd, args, ctx: { c, cloudRequest, resolveTaskId } });
    if (handledGate) return true;
  }

  {
    const { maybeHandleChatCommand } = require('../commands/chat');
    const handledChat = await maybeHandleChatCommand({ cmd, args, ctx: { c, ensureBoardRunning, probeBoardHealth, getBoardPort, setBoardEnsuredFalse: resetBoardEnsured } });
    if (handledChat) return true;

    const { maybeHandleMeshCommand } = require('../commands/mesh');
    const handledMesh = await maybeHandleMeshCommand({ cmd, args, ctx: { c } });
    if (handledMesh) return true;

    const { maybeHandleUpdateCommand } = require('../commands/update');
    const handledUpdate = await maybeHandleUpdateCommand({ cmd, args, ctx: { c, stopDaemon } });
    if (handledUpdate) return true;
  }
  ...
  if (cmd === 'run' || (cmd === 'task' && args[1] === 'run')) {
    ...
    if (!taskId) {
      console.log(`${c.yellow}Usage:${c.reset} agx run <taskId> [--task <id>] [--swarm] [--no-memory]`);
      console.log(`${c.dim}   or:${c.reset} agx task run <taskId> [--task <id>] [--swarm] [--no-memory]`);
      process.exit(1);
    }

    try {
      const exitCode = await runTaskInline(taskId, { forceSwarm, noMemory });
  ...
  // agx memory list [--agent <id>]
  if (cmd === 'memory' && (args[1] === 'list' || args[1] === 'ls')) {
    const { getMemoriesByAgent, openDb } = require('../storage/db');
    let agentId = null;
    ...
    if (!agentId) {
      agentId = args.slice(2).find(a => !a.startsWith('-')) || null;
    }

    try {
      openDb(); // ensure DB exists
      if (agentId) {
        const memories = getMemoriesByAgent(agentId);
        if (memories.length === 0) {
          console.log(`${c.dim}No memories for agent "${agentId}"${c.reset}`);
        } else {
          console.log(`${c.green}${memories.length}${c.reset} memories for agent "${agentId}":\n`);
          for (const mem of memories) {
            const date = new Date(mem.created_at).toISOString().slice(0, 10);
            console.log(`  ${c.dim}${date}${c.reset} [${c.cyan}${mem.memory_type}${c.reset}] ${mem.content}`);
          }
        }
      } else {
        // List all memories grouped by agent
        const db = openDb();
        const all = db.prepare('SELECT * FROM agent_memory ORDER BY agent_id, created_at DESC').all();
        if (all.length === 0) {
          console.log(`${c.dim}No memories stored yet.${c.reset}`);
        } else {
          const grouped = {};
          for (const mem of all) {
            (grouped[mem.agent_id] = grouped[mem.agent_id] || []).push(mem);
          }
          for (const [aid, mems] of Object.entries(grouped)) {
            console.log(`\n${c.green}${aid}${c.reset} (${mems.length} memories):`);
  ...
  // agx memory clear [--agent <id>]
  if (cmd === 'memory' && args[1] === 'clear') {
    ...
    try {
      ...
      if (agentId) {
        ...
        console.log(`${c.green}✓${c.reset} Cleared ${result.changes} memories for agent "${agentId}"`);
      } else {
        const result = db.prepare('DELETE FROM agent_memory').run();
        console.log(`${c.green}✓${c.reset} Cleared all ${result.changes} memories`);
      }
    } catch (err) {
      console.log(`${c.red}✗${c.reset} Failed: ${err.message}`);
      process.exit(1);
  ...
  if (cmd === 'feedback') {
    const readline = require('readline');
    const FEEDBACK_API_URL = 'https://www.runagx.com/api/feedback/add';
    const FEEDBACK_FORM_URL = 'https://www.runagx.com/feedback';
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(payload?.error || `Feedback API failed with status ${response.status}`);
      }

      console.log(`\n${c.green}✓${c.reset} Feedback saved to the AGX board`);
      const savedTitle = payload?.item?.title;
      if (savedTitle) {
        console.log(`${c.dim}${savedTitle}${c.reset}`);
      }
      console.log(`${c.dim}${FEEDBACK_FORM_URL}${c.reset}`);
    } catch (err) {
      const message = err?.name === 'AbortError'
        ? 'Timed out while contacting the feedback API.'
        : (err?.message || String(err));
      console.error(`\n${c.red}✗${c.reset} Unable to submit feedback: ${message}`);
      console.log(`${c.cyan}Submit manually at:${c.reset} ${FEEDBACK_FORM_URL}`);
      process.exit(1);
      return true;
    } finally {
...
async function runCli(argv = process.argv) {
  ...
  try {
    ...
    const VALID_PROVIDERS = ['gemini', 'claude', 'ollama', 'codex', 'zai', 'kimi'];
...
OPTIONS:
  ...
  -y, --yolo          Skip prompts during execution (implied by -a, not used with 'new')
  -m, --model         Model name
...
PROVIDERS:
  ...
  zai, z       Z.AI GLM models (via Claude Code)
  kimi, k      Moonshot Kimi Code
  ...
  agx approve <task>     Approve an awaiting gate (auto-finds node)
  agx reject <task>      Reject an awaiting gate
  agx approve <task> --node <id> -m "feedback"
  agx repo add . --project <project>  Analyze current repo and attach it to a project
  agx workspace list [--project <project>]     Show the workspace map
  agx workspace add repos backend /code/api    Add a workspace entry
...
EXAMPLES:
  ...
  agx zai -p "hello"              # Use Z.AI (GLM) models
  agx task ls               # Check cloud tasks
  agx container logs         # See what's happening

SCHEDULING:
  agx schedule ls             List scheduled jobs
  agx schedule create "name"  Create a scheduled job (--cadence, --prompt/--prompt-file)
  agx schedule get <id>       Show schedule details
  agx schedule update <id>    Update a schedule (--name, --cadence, --prompt, etc.)
  agx schedule pause <id>     Pause a schedule
  agx schedule resume <id>    Resume a schedule
  agx schedule rm <id>        Delete a schedule
  agx schedule runs <id>      List run history
  agx schedule cancel <id>    Cancel active run

PR NARRATIVES:
  agx narrate-prs [path]      Analyze merged PRs as product/marketing narrative (incremental)
  agx narrate-prs ls [path]   Show storage, state, and file counts
  agx narrate-prs where [path] Print storage directory
  agx narrate-prs cat [path]   Concatenate narratives (--from N --to N --output file)
    Flags: --provider, --model, --concurrency, --limit, --since, --force,
           --dry-run, --prompt-file, --output-dir

REMOTE ACCESS:
  agx board share            Enable remote access via Tailscale
  agx board share --off      Disable remote access
  agx board share --status   Show remote access status
  ...
  agx feedback               Report a bug (auto-collects context)`);
    ...
    if (!provider || (!isProviderArg && provider.startsWith('-'))) {
      if (config?.defaultProvider) {
        ...
        console.log(`  ${c.dim}agx zai --prompt "hello"${c.reset}\n`);
        process.exit(1);
      }
    }
    ...
    if (provider === 'gemini') {
      ...
      // Z.AI routes through Claude CLI with z.ai base URL and GLM model mappings
      command = 'claude';
      translatedArgs.length = 0; // Clear any accumulated args

      const zaiKey = config?.zai?.apiKey || process.env.ZAI_API_KEY || '';
      if (!zaiKey) {
        console.error(`${c.red}Error:${c.reset} No Z.AI API key configured.`);
        console.error(`Run ${c.cyan}agx config${c.reset} and choose "Login to a provider" → zai, or set ${c.cyan}ZAI_API_KEY${c.reset}.`);
        process.exit(1);
      }
      ...
      // Kimi Code CLI mirrors Claude's flag surface (--print, -p, --model).
      command = 'kimi';
    ...
    // Resolve command to absolute path for non-interactive shells (SSH, daemon)
    { const { resolveEngineCmd } = require('../executor'); command = resolveEngineCmd(command); }
    ...
    // Safe cwd: process.cwd() can throw ENOENT if the directory was deleted (e.g. daemon/chat)
    let safeCwd;
      ...
      childProc.on('error', (err) => {
        if (err.code === 'ENOENT') {
          ...
          if (cmd === 'claude') {
            ...
            console.error(`  See https://www.kimi.com/code/docs`);
          }
        } else {
