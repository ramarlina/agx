...
async function showConfigStatus() {
  ...
  if (config) {
    ...
    console.log(`  Cloud URL: ${c.cyan}${cloudUrl}${c.reset}`);
  } else {
...
async function runConfigMenu() {
  ...
  console.log(`  ${c.cyan}4${c.reset}) Set cloud URL`);
  ...
  switch (choice) {
    ...
    case '4': {
      ...
      console.log(`\n  Current cloud URL: ${c.cyan}${currentUrl}${c.reset}`);
      ...
      if (newUrl && newUrl.trim()) {
        ...
        console.log(`\n${c.green}✓${c.reset} Cloud URL set to ${c.cyan}${newUrl.trim()}${c.reset}`);
      } else {
