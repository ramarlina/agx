...
const DEFAULT_AGENT = { name: 'Sage', identity: 'A thoughtful, methodical thinker who reasons carefully before acting' };

function createDefaultParticipant(provider, model) {
  const agent = DEFAULT_AGENT;
  const storeDir = path.join(os.homedir(), '.agx', 'group-chat');
  const dbPath = path.join(storeDir, 'history.sqlite');
  ...
  try {
    ...
    db.prepare(
      'INSERT INTO participants (id, name, provider, model, identity, color, sort_order) VALUES (?, ?, ?, ?, ?, ?, 0)'
    ).run(id, agent.name, provider, model, agent.identity, '#D97706');
    db.close();

    console.log(`${c.green}✓${c.reset} Created chat agent: ${c.bold}${agent.name}${c.reset} (${c.cyan}${provider}${c.reset} / ${c.cyan}${model}${c.reset})`);
  } catch (err) {
    // Non-fatal — onboarding should still succeed
    console.log(`${c.dim}  (skipped chat agent creation: ${err.message})${c.reset}`);
  }
}
...
`);
  ...
  if (defaultProvider === 'ollama') {
    const ollamaModels = ['glm-5:cloud', 'qwen3.5:397b-cloud', 'qwen3.5:cloud', 'minimax-m2.5:cloud', 'kimi-k2.5:cloud'];
    console.log(`\n${c.bold}Choose your default model:${c.reset}`);
    ollamaModels.forEach((m, i) => {
      console.log(`  ${c.cyan}${i + 1}${c.reset}) ${m}`);
    });
    console.log(`  ${c.cyan}6${c.reset}) Other`);
    while (!defaultModel) {
      const choice = await prompt(`\nEnter number [${c.dim}1${c.reset}]: `);
      if (!choice) {
        ...
        if (custom && custom.trim()) defaultModel = custom.trim();
      } else if (/^[1-5]$/.test(choice)) {
        defaultModel = ollamaModels[parseInt(choice, 10) - 1];
      }
    }
  } else if (defaultProvider === 'claude') {
    const claudeModels = ['claude-opus-4-6', 'claude-sonnet-4-6', 'claude-haiku-4-5'];
    console.log(`\n${c.bold}Choose your default model:${c.reset}`);
    claudeModels.forEach((m, i) => {
      console.log(`  ${c.cyan}${i + 1}${c.reset}) ${m}`);
    });
    console.log(`  ${c.cyan}4${c.reset}) Other`);
    while (!defaultModel) {
      const choice = await prompt(`\nEnter number [${c.dim}1${c.reset}]: `);
      if (!choice) {
        defaultModel = claudeModels[0];
      } else if (choice === '4') {
        const custom = await prompt(`\nEnter model name: `);
        if (custom && custom.trim()) defaultModel = custom.trim();
      } else if (/^[1-3]$/.test(choice)) {
        defaultModel = claudeModels[parseInt(choice, 10) - 1];
      }
    }
  } else if (defaultProvider === 'codex') {
    const codexModels = ['gpt-5.4', 'gpt-5.3-codex', 'gpt-5.1-code-mini'];
    console.log(`\n${c.bold}Choose your default model:${c.reset}`);
    codexModels.forEach((m, i) => {
      console.log(`  ${c.cyan}${i + 1}${c.reset}) ${m}`);
    });
    console.log(`  ${c.cyan}${codexModels.length + 1}${c.reset}) Other`);
    while (!defaultModel) {
      const choice = await prompt(`\nEnter number [${c.dim}1${c.reset}]: `);
      if (!choice) {
        defaultModel = codexModels[0];
      } else if (choice === String(codexModels.length + 1)) {
        const custom = await prompt(`\nEnter model name: `);
        if (custom && custom.trim()) defaultModel = custom.trim();
      } else {
        const n = parseInt(choice, 10);
        if (Number.isInteger(n) && n >= 1 && n <= codexModels.length) {
          defaultModel = codexModels[n - 1];
        }
      }
    }
  } else if (defaultProvider === 'zai') {
    const zaiModels = ['glm-5.1', 'glm-5-turbo', 'glm-4.5-air'];
    console.log(`\n${c.bold}Choose your default model:${c.reset}`);
    zaiModels.forEach((m, i) => {
      console.log(`  ${c.cyan}${i + 1}${c.reset}) ${m}`);
    });
    console.log(`  ${c.cyan}${zaiModels.length + 1}${c.reset}) Other`);
    while (!defaultModel) {
      const choice = await prompt(`\nEnter number [${c.dim}1${c.reset}]: `);
      if (!choice) {
        defaultModel = zaiModels[0];
      } else if (choice === String(zaiModels.length + 1)) {
        const custom = await prompt(`\nEnter model name: `);
        if (custom && custom.trim()) defaultModel = custom.trim();
      } else {
        const n = parseInt(choice, 10);
        if (Number.isInteger(n) && n >= 1 && n <= zaiModels.length) {
          defaultModel = zaiModels[n - 1];
        }
      }
    }
  } else if (defaultProvider === 'gemini') {
    const geminiModels = ['gemini-3-pro-preview', 'gemini-3-flash-preview'];
    console.log(`\n${c.bold}Choose your default model:${c.reset}`);
    geminiModels.forEach((m, i) => {
      console.log(`  ${c.cyan}${i + 1}${c.reset}) ${m}`);
    });
    console.log(`  ${c.cyan}3${c.reset}) Other`);
    while (!defaultModel) {
      const choice = await prompt(`\nEnter number [${c.dim}1${c.reset}]: `);
      if (!choice) {
        ...
        const custom = await prompt(`\nEnter model name: `);
        if (custom && custom.trim()) defaultModel = custom.trim();
      } else if (/^[1-2]$/.test(choice)) {
        defaultModel = geminiModels[parseInt(choice, 10) - 1];
      }
    }
  } else {
    while (!defaultModel) {
  ...
  const config = {
    ...
    providerPaths: Object.fromEntries(Object.entries(providers).filter(([_, v]) => v)),
  };
  ...
  createDefaultParticipant(defaultProvider, defaultModel);
...
${c.bold}Quick Start:${c.reset}
  ...
  ${c.dim}# Open the UI${c.reset}
  ${c.cyan}agx board${c.reset}
...
`);
  ...
  const openUi = await prompt(`Open UI now? [Y/n]: `);
  if (!openUi || openUi.toLowerCase() !== 'n') {
    const { spawn } = require('child_process');
    const child = spawn(process.argv[0], [process.argv[1], 'board'], {
      stdio: 'inherit',
      detached: true,
    });
    child.unref();
...
async function showConfigStatus() {
  ...
  if (config) {
    ...
    if (config?.providerPaths && Object.keys(config.providerPaths).length > 0) {
      console.log(`\n  ${c.bold}Provider paths:${c.reset}`);
      for (const [name, p] of Object.entries(config.providerPaths)) {
        console.log(`    ${c.cyan}${name}${c.reset}: ${c.dim}${p}${c.reset}`);
      }
    }
...
async function promptAndSetModel(provider, currentConfig) {
  const currentModel = currentConfig?.models?.[provider] || '(none)';
  console.log(`\n  Provider: ${c.cyan}${provider}${c.reset}`);
  console.log(`  Current model: ${c.cyan}${currentModel}${c.reset}`);

  let model = '';
  let verified = false;
  while (!verified) {
    const input = await prompt(`\n  New model${model ? '' : ` [${c.dim}${currentModel}${c.reset}]`}: `);
    model = (input && input.trim()) ? input.trim() : (model || currentModel);
    if (model === '(none)') {
      console.log(`\n${c.yellow}No model specified.${c.reset}`);
      continue;
    }
    ...
    const newConfig = {
      ...(currentConfig || {}),
      models: { ...(currentConfig?.models || {}), [provider]: model },
      ...(provider === 'ollama' ? { ollama: { ...(currentConfig?.ollama || {}), model } } : {}),
    };
    saveConfig(newConfig);
    console.log(`\n${c.green}✓${c.reset} Default model for ${c.cyan}${provider}${c.reset} set to ${c.cyan}${model}${c.reset}`);

    // Smoke test
    const smoke = await runAgxModelSmokeTest({ provider, model });
    if (smoke.code === 0 && !smoke.timedOut) {
      console.log(`${c.green}✓${c.reset} Model verified`);
      verified = true;
    } else {
      const errSnippet = (smoke.stderr || smoke.stdout || '').trim().split('\n').slice(-3).join('\n');
      console.log(`\n${c.red}✗${c.reset} Smoke test failed${smoke.timedOut ? ' (timed out)' : ` (exit ${smoke.code})`}`);
      if (errSnippet) console.log(`${c.dim}${errSnippet}${c.reset}`);
      const retry = await prompt(`\n  Try a different model? (Y/n): `);
      if (retry.toLowerCase() === 'n') {
        verified = true; // keep the model anyway
      }
    }
  }
}
...
async function runConfigMenu() {
  ...
  console.log(`  ${c.cyan}4${c.reset}) Change default model`);
  console.log(`  ${c.cyan}5${c.reset}) Set backend URL`);
  console.log(`  ${c.cyan}6${c.reset}) Show status`);
  console.log(`  ${c.cyan}7${c.reset}) Run full setup wizard`);
  ...
  switch (choice) {
    case '1': {
      const missing = ['claude', 'gemini', 'ollama', 'codex', 'zai'].filter((p) => !providers[p]);
      if (missing.length === 0) {
    ...
    case '3': {
      ...
      if (idx >= 0 && idx < installed.length) {
        ...
        // Prompt for model
        await promptAndSetModel(installed[idx], newConfig);
      }
      break;
    }
    case '4': {
      ...
      if (!provider) {
        console.log(`\n${c.yellow}No default provider set.${c.reset} Set one first.`);
        break;
      }
      await promptAndSetModel(provider, config);
      break;
    }
    case '5': {
      const cloudConfig = loadCloudConfigFile();
