...
`);
  ...
  if (defaultProvider === 'ollama') {
    ...
    const codexModels = ['gpt-5.3-codex', 'gpt-5.1-code-mini'];
    console.log(`\n${c.bold}Choose your default model:${c.reset}`);
    ...
    while (!defaultModel) {
      ...
      if (!choice) {
        ...
        defaultModel = codexModels[parseInt(choice, 10) - 1];
      }
    }
...
${c.bold}Quick Start:${c.reset}
  ...
  ${c.dim}# Chat with your agents${c.reset}
  ${c.cyan}agx chat${c.reset}
...
`);
  ...
  const openChat = await prompt(`Open chat now? [Y/n]: `);
  if (!openChat || openChat.toLowerCase() !== 'n') {
    const { spawn } = require('child_process');
    const child = spawn(process.argv[0], [process.argv[1], 'chat'], {
      stdio: 'inherit',
      detached: true,
    });
    child.unref();
