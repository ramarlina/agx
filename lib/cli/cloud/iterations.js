...
function createCloudIterationHelpers(env) {
  ...
  async function runSingleAgentPlanIteration({ taskId, provider, model, prompt, logger, onStdout, onStderr, artifacts, cancellationWatcher, env, cwd }) {
    logExecutionFlow('runSingleAgentPlanIteration', 'input', `taskId=${taskId}, provider=${provider}, model=${model}, prompt=${Boolean(prompt) ? 'present' : 'none'}`);
    logExecutionFlow('runSingleAgentPlanIteration', 'processing', 'preparing runAgxCommand');
    // Plan generation must be output-only: no --cloud-task context injection.
    // This avoids execution-oriented instructions and keeps the planner deterministic.
    const args = [provider, '-y'];
    if (model) {
      args.push('--model', model);
    }
    if (prompt) {
      const planPrompt = String(prompt);
      artifacts?.recordPrompt(`Plan Iteration Prompt (${provider}${model ? `/${model}` : ''})`, planPrompt);
      args.push('--prompt', planPrompt);
    }
    args.push('--print');

    await abortIfCancelled(cancellationWatcher);
    const res = await pRetryFn(
      () => runAgxCommand(args, SWARM_TIMEOUT_MS, `agx ${provider} [plan]`, {
        env,
        cwd,
        onStdout: (data) => {
          if (onStdout) onStdout(data);
        ...
        onTrace: (event) => {
          void artifacts?.recordEngineTrace?.({ provider, model: model || null, role: 'plan-iteration' }, event);
    ...
    const outputSource = res.stdout || res.stderr || '';
    logExecutionFlow('runSingleAgentPlanIteration', 'output', `response length=${outputSource.length}`);
    return outputSource;
  }

  return { runSwarmIteration, runSingleAgentIteration, runSingleAgentPlanIteration };
}
