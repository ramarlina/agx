...
const { commandExists, resolveCommand } = require('../proc/commandExists');
const { prompt, loadConfig, saveConfig } = require('./configStore');

function detectProviders() {
  const claudePath = resolveCommand('claude') || false;
  const config = loadConfig();
  const hasZaiKey = Boolean(config?.zai?.apiKey);
  return {
    claude: claudePath,
    gemini: resolveCommand('gemini') || false,
    ollama: resolveCommand('ollama') || false,
    codex: resolveCommand('codex') || false,
    // z.ai routes through Claude CLI — "installed" means claude CLI + key configured
    zai: claudePath && hasZaiKey ? claudePath : false,
  };
}
...
function printProviderStatus(providers) {
  ...
  console.log(`  ${c.cyan}zai${c.reset}     │ Z.AI (GLM via Claude)  │ ${status(providers.zai)}`);
}
...
const PROVIDERS = {
  ...
  zai: {
    name: 'Z.AI',
    installCmd: 'npm install -g @anthropic-ai/claude-code',
    description: 'Z.AI GLM models via Claude Code'
  }
};
...
async function installProvider(provider) {
  ...
  // z.ai "install" just means configuring the API key (claude CLI is the actual binary)
  if (provider === 'zai') {
    if (!commandExists('claude')) {
      console.log(`\n${c.cyan}Installing Claude Code (required for z.ai)...${c.reset}\n`);
      console.log(`${c.dim}$ ${info.installCmd}${c.reset}\n`);
      await runInteractive(info.installCmd);
      if (!commandExists('claude')) {
        console.log(`\n${c.red}✗${c.reset} Claude CLI install failed.`);
        return false;
      }
    }
    return loginProvider('zai');
  }
...
async function loginProvider(provider) {
  ...
  if (provider === 'zai') {
    if (!commandExists('claude')) {
      console.log(`${c.yellow}Claude CLI is required for z.ai.${c.reset}`);
      console.log(`${c.dim}Install: npm install -g @anthropic-ai/claude-code${c.reset}`);
      return false;
    }
    console.log(`${c.cyan}Configure Z.AI access${c.reset}`);
    console.log(`${c.dim}Get your API key from https://z.ai/manage-apikey/apikey-list${c.reset}\n`);
    const key = (await prompt('  Enter Z.AI API key: ')).trim();
    if (!key) {
      console.log(`${c.yellow}No key entered. Skipping.${c.reset}`);
      return false;
    }
    const config = loadConfig() || {};
    saveConfig({ ...config, zai: { ...(config.zai || {}), apiKey: key } });
    console.log(`\n${c.green}✓${c.reset} Z.AI API key saved to ${c.dim}~/.agx/config.json${c.reset}`);
    return true;
  }
