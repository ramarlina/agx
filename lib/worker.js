...
class AgxWorker {
  constructor(config) {
    ...
    this.maxConcurrent = parsePositiveInt(
      config.daemonMaxConcurrent ?? process.env.AGX_DAEMON_MAX_CONCURRENT ?? 1,
      1
    );
  ...
  async start() {
    ...
    console.log(`${c.dim}   Workers: ${this.maxConcurrent}${c.reset}`);
  ...
  async pollOnce() {
    ...
    // Pull tasks up to available worker capacity.
    while (this.isRunning && this.hasCapacity()) {
      try {
        const { task } = await this.apiRequest('GET', `/api/queue`);
        if (!task) break;
        const accepted = await this.tryClaimAndExecute(task);
        if (!accepted) break;
      } catch {
        break;
      }
  ...
  async tryClaimAndExecute(task) {
    ...
    if (this.knownTaskIds.has(taskId) || this.activeTasks.has(taskId)) {
      return false;
    }
    ...
    if (!this.hasCapacity()) {
      ...
      console.log(`${c.dim}Queued locally, waiting for worker slot: ${taskId}${c.reset}`);
      return true;
    }
  ...
  hasCapacity() {
    if (this.maxConcurrent === 1 && this.currentTask) return false;
    return this.activeTasks.size < this.maxConcurrent;
  }

  dispatchTask(task) {
    const taskId = task.id;
    this.activeTasks.set(taskId, { task, child: null, startedAt: Date.now() });
    this.knownTaskIds.add(taskId);
  ...
  drainPendingQueue() {
    while (this.hasCapacity() && this.pendingTaskQueue.length > 0) {
      const task = this.pendingTaskQueue.shift();
      if (!task?.id || this.activeTasks.has(task.id)) continue;
      this.dispatchTask(task);
    }
  }
  ...
  finalizeTask(taskId) {
    ...
    if (this.currentTask?.id === taskId) {
      const nextActive = this.activeTasks.values().next();
      this.currentTask = nextActive.done ? null : nextActive.value.task;
    }
    this.drainPendingQueue();
    if (this.isRunning) {
      setTimeout(() => {
  ...
  async executeViaRunCommand(taskId) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.registerChild(taskId, child);
  ...
  async stop() {
    ...
    if (this.currentTask || this.activeTasks.size > 0) {
      console.log(`${c.yellow}⏳${c.reset} Waiting for ${this.activeTasks.size || 1} active task(s) to finish...`);
      // Wait for active work (max 30s)
      let waited = 0;
      while ((this.currentTask || this.activeTasks.size > 0) && waited < 30000) {
        await new Promise(r => setTimeout(r, 1000));
        waited += 1000;
      }

      if (this.activeTasks.size > 0) {
        for (const { child } of this.activeTasks.values()) {
