    ...
    try {
      const response = await fetch(`${cloudApiUrl}/api/auth/daemon-secret`, {
        ...
        body: JSON.stringify({ 
          ...
          // We send a hash of the secret, not the secret itself
          // Cloud stores the hash for signature verification
        }),
      });
...
async function securityCheck(task, options = {}) {
  ...
  if (requireSignature && task.signature) {
    ...
    const valid = verifyTaskSignature(task, task.signature, secret);
