...
import type { TrackerRunScript } from "@/state/trackerRunScripts";
...
interface RunScriptManagerProps {
  ...
  onSaveScript: (input: { id?: string; name: string; prompt: string }) => TrackerRunScript;
...
function buildProjectLabel(projectSlug?: string | null): string {
  return String(projectSlug ?? "").trim() || "global tracker board";
}
...
export default function RunScriptManager({
  ...
  const [deleteTarget, setDeleteTarget] = useState<TrackerRunScript | null>(null);
  ...
  const selectScript = (script: TrackerRunScript) => {
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="overflow-hidden rounded-lg border border-[var(--card-border)] bg-[var(--background)] transition-colors focus-within:border-blue-500 [&_.rich-text-editor_.ProseMirror]:min-h-[220px]">
                <RichTextEditor
                  ...
                  placeholder={"Read {{ticket.identifier}} and the full comment thread before changing code.\nWrite your investigation to {{knowledge_base.issue_path}} and stop after posting a plan unless the item is already approved for implementation."}
                />
              </div>
            </div>
