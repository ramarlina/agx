...
export function useTrackerActiveAgents(
  trackerType: string,
  projectId?: string,
): {
  issueActiveAgents: Map<string, Array<{ agentId: string; agentName: string }>>;
} {
  const [issueActiveAgents, setIssueActiveAgents] = useState<
    Map<string, Array<{ agentId: string; agentName: string }>>
  >(new Map());
    ...
    async function fetchActiveAgents() {
      try {
        ...
        const res = await fetch(
          `/api/trackers/${trackerType}/active-agents?${params}`,
        );
