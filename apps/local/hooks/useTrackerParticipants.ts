...
export function useTrackerParticipants(trackerType: string, projectId?: string): {
  ...
  useEffect(() => {
    ...
    async function loadParticipants() {
      try {
        ...
        // Check if the tracker worker has a team configured
        const workerParams = new URLSearchParams();
        workerParams.set("projectId", projectId);
        const workerResponse = await fetch(`/api/trackers/${encodeURIComponent(trackerType)}/worker?${workerParams}`).catch(() => null);
