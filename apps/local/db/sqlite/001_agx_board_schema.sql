-- AGX Board schema — SQLite port of docker/postgres/init/001_agx_board_schema.sql
-- Ported: 2026-02-21
-- Type mapping: UUID→TEXT, TIMESTAMPTZ→TEXT (ISO-8601), JSONB→JSON, uuid[]/text[]→JSON, inet→TEXT, numeric→REAL
-- Schema freeze: This file is the canonical SQLite DDL. Do not modify the Postgres
--   source schema until both files are verified in sync. Unfreeze criteria:
--   1. sqlite3 :memory: < this_file succeeds with zero errors
--   2. Table/column name diff between PG and SQLite DDLs shows no mismatches
--   3. This header is updated with the unfreeze date

PRAGMA journal_mode = WAL;
PRAGMA foreign_keys = ON;

-- ── Helper: UUID generation ─────────────────────────────────────────────────
-- SQLite has no gen_random_uuid(). We use randomblob() to build a v4 UUID string.
-- Usage: DEFAULT (agx_uuid())
-- NOTE: This is a workaround; callers may also supply UUIDs from application code.
...
-- GIN index on stage_decisions not supported in SQLite; use regular index for lookups
...
-- Partial indexes for task_templates (SQLite supports WHERE on indexes)
...
-- with recursive_triggers OFF (the default) to prevent infinite recursion.
