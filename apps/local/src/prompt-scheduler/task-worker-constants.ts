// Task worker constants — tracker-agnostic replacement for linear-worker-constants.
// Phase 1: These mirror the Linear constants with updated naming.
// Phase 2: When Jira/GitHub adapters are added, the prompt text will become
//   tracker-aware (e.g., referencing "the issue tracker MCP" instead of "Linear MCP").

export const TASK_WORKER_JOB_NAME = "Task worker";
export const TASK_WORKER_DEFAULT_CADENCE = "every 30 minutes";

/**
 * Default selection prompt — guides the controller LLM when picking which ticket to work on.
 */
export const TASK_WORKER_DEFAULT_PROMPT = `Pick the highest-priority actionable ticket. Prefer tickets that:
 ...
 * When empty, the system falls back to the built-in execution prompt (buildTrackerExecutionPrompt).
 ...
 */
export const TASK_WORKER_DEFAULT_SCRIPT_PROMPT = `Work on {{ticket.identifier}}: {{ticket.title}}
