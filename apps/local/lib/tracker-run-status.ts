...
export type RunDisplayTone = TrackerRunRecord["status"] | "ready";

export function formatRunStatus(status: TrackerRunRecord["status"]): string {
  switch (status) {
    case "queued":
      return "queued";
    case "running":
...
export function getRunDisplayState(run: TrackerRunRecord): {
  label: string;
  tone: RunDisplayTone;
} {
  if (run.mode === "chat") {
...
export function getRunTitle(run: TrackerRunRecord): string {
