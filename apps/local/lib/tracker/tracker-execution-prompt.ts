...
// Inlined from the deleted linear-execution-prompt.ts. The language is
// already tracker-agnostic (references "issue tracker" instead of "Linear").

export interface TrackerExecutionIssueContext {
  identifier: string;
  title: string;
  status?: string | null;
...
export interface TrackerExecutionProjectRepoContext {
  name: string;
  path?: string | null;
  notes?: string | null;
}

export interface TrackerExecutionProjectContext {
  name?: string | null;
  slug?: string | null;
  repos?: TrackerExecutionProjectRepoContext[] | null;
}

export interface TrackerExecutionRuntimeContext {
...
export interface TrackerExecutionPromptInput {
  issue: TrackerExecutionIssueContext;
  project?: TrackerExecutionProjectContext | null;
  runtime?: TrackerExecutionRuntimeContext | null;
}

export interface ResolvedTrackerExecutionContext {
  issue: {
    identifier: string;
    title: string;
    status: string;
  ...
  project: {
    ...
    slug: string;
    label: string;
    repos: TrackerExecutionProjectRepoContext[];
  };
  runtime: {
...
function toProjectSlug(project?: TrackerExecutionProjectContext | null): string {
...
function toProjectLabel(project?: TrackerExecutionProjectContext | null): string {
...
function formatRepositories(repos?: TrackerExecutionProjectRepoContext[] | null): string {
...
export function buildTrackerExecutionPrompt(input: TrackerExecutionPromptInput): {
  prompt: string;
  promptPrefix: string;
} {
  const context = resolveTrackerExecutionContext(input);
...
export function resolveTrackerExecutionContext(
  input: TrackerExecutionPromptInput
): ResolvedTrackerExecutionContext {
...
export function renderTrackerExecutionPromptTemplate(
  template: string,
  input: TrackerExecutionPromptInput
): string {
  const context = resolveTrackerExecutionContext(input);
