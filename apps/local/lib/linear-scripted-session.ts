...
export interface ScriptedLinearSessionIssueInput {
  id: string;
  identifier: string;
  title: string;
  status: string;
...
export interface StartScriptedLinearSessionInput {
  projectId?: string | null;
  projectSlug?: string | null;
  issue: ScriptedLinearSessionIssueInput;
  agentId: string;
  scriptName?: string | null;
  scriptPrompt?: string | null;
}

export interface StartedScriptedLinearSession {
  run: LinearRunRecord;
  chatRunId: string;
  userMessageId: string;
}

export async function startScriptedLinearSession(
  input: StartScriptedLinearSessionInput
  ...
  try {
    const promptInput: LinearExecutionPromptInput = {
      issue: {
    ...
    const prompt = customScriptPrompt
      ? renderLinearExecutionPromptTemplate(customScriptPrompt, promptInput).trim() || defaultPrompt
      : defaultPrompt;
