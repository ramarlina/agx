...
const { scheduleTermination, isPidAlive: isPidAliveProc } = require('./lib/proc/killProcessTree');
const {
  collectProjectFlags,
  buildProjectBody,
  createProject,
...
// If this process was spawned by another agx process, exit if the parent disappears.
// This prevents orphaned provider runners after daemon crashes or is force-killed.
(() => {
  const rawParentPid = process.env.AGX_PARENT_PID;
  const parentPid = rawParentPid ? parseInt(String(rawParentPid), 10) : 0;
  if (!parentPid || parentPid <= 1 || parentPid === process.pid) return;

  // If the parent is already gone, don't start.
  if (!isPidAliveProc(parentPid)) {
    process.exit(97);
  }
...
function dockerHasRelation(qualifiedName) {
  ...
  // Output is a 2-row table. We just need to know it's non-empty and not null.
...
async function ensurePostgresReady() {
  ...
  if (boardEnv.DATABASE_URL) {
    ...
    try {
      ...
      if (result.status === 0) {
        // If user is using the bundled docker Postgres, ensure schema exists even if the container
        // was started previously (or schema init timed out).
...
async function runAgxModelSmokeTest({ provider, model }) {
  ...
  return await new Promise((resolve) => {
    ...
    const MAX_CAPTURE = 32_000; // prevent unbounded memory
...
`);
  ...
  // Ask for default model (used when --model is not specified)
  let defaultModel = '';
  if (defaultProvider === 'ollama') {
    ...
    while (!defaultModel) {
      if (models.length > 0) {
        ...
        // Ollama installs are handled earlier in loginProvider; fall back to manual entry.
  ...
  // Smoke test: verify the configured provider/model can answer a trivial prompt
...
async function checkOnboarding() {
  ...
  if (cmd === 'board') {
    ...
    if (!subcmd || subcmd === 'start') {
      ...
      console.log(`  agx board migrate      Run database schema migrations`);
      ...
      const boardEnv = loadBoardEnv();
      const dbUrl = boardEnv.DATABASE_URL;
      if (!dbUrl) {
        console.error(`${c.red}No DATABASE_URL configured.${c.reset} Run ${c.bold}agx board start${c.reset} first or set DATABASE_URL in board env.`);
        process.exit(1);
      }
      ensureSchemaInitialized(dbUrl);
      console.log(`${c.green}✓${c.reset} Schema migrations complete`);
      process.exit(0);
    } else if (subcmd === 'status') {
      const pid = isBoardRunning();
      if (pid) {
      ...
      console.log(`${c.dim}Usage: agx board [start|stop|status|show|open|logs|tail|migrate]${c.reset}`);
      process.exit(0);
    }
    return true;
  }
